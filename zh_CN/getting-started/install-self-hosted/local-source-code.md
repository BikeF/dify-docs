...
> 若需要使用 OpenAI TTS，需要在系统中安装 FFmpeg 才可正常使用，详情可参考：[Link](https://docs.kofe.ai/v/zh-hans/learn-more/faq/install-faq#id-15.-wen-ben-zhuan-yu-yin-yu-dao-zhe-ge-cuo-wu-zen-me-ban)。
...
4.  安装依赖包
    ...
    Kofe API 服务使用 [Poetry](https://python-poetry.org/docs/) 来管理依赖。您可以执行 `poetry shell` 来激活环境。
...
7.  启动 Worker 服务
    ...
    - ** ---------- .> results:     postgresql://postgres:**@localhost:5432/kofe
...
3.  配置环境变量。在当前目录下创建文件 `.env.local`，并复制`.env.example`中的内容。根据需求修改这些环境变量的值:
    ...
    # example: http://cloud.kofe.ai/console/api
...
最后，访问 http://127.0.0.1:3000 即可使用本地部署的 Kofe。
