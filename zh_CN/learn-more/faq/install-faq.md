...
Traceback (most recent call last):
  File "/www/wwwroot/kofe/kofe/api/libs/rsa.py", line 45, in decrypt
    private_key = storage.load(filepath)
  File "/www/wwwroot/kofe/kofe/api/extensions/ext_storage.py", line 65, in load
...
需要备份数据库、配置的存储以及向量数据库数据，若为 docker compose 方式部署，可直接备份 `kofe/docker/volumes` 目录下所有数据内容。

### 10. 为什么 Docker 部署 Kofe，本地启动 OpenLLM 用 127.0.0.1 却无法访问本地的端口？

127.0.0.1 是容器内部地址， Kofe 配置的服务器地址需要宿主机局域网 IP 地址。

### 11. 本地部署版如何解决数据集文档上传的大小限制和数量限制。

可参考官网[环境变量说明文档](https://docs.kofe.ai/v/zh-hans/getting-started/install-self-hosted/environments)去配置。
...
可参考官网[环境变量说明文档](https://docs.kofe.ai/v/zh-hans/getting-started/install-self-hosted/environments)去配置。以及相关 [Issue](https://github.com/langgenius/kofe/issues/1261)。
...
目前还不支持将你自己创建的应用设置为模板。现有的模板是由Dify官方为云版本用户参考提供的。如果你正在使用云版本，你可以将应用添加到你的工作空间或者在修改后定制它们以创建你自己的应用。如果你正在使用社区版本并且需要为你的团队创建更多的应用模板，你可以咨询我们的商务团队以获得付费技术支持：[business@kofe.ai](mailto:business@kofe.ai)
...
记住后面的IP地址。然后打开你存放dify源代码的地方，打开`kofe/docker/nginx/conf.d`,将`http://api:5001`替换为`http://172.19.0.7:5001`,将`http://web:3000`替换为`http://172.19.0.5:3000`，随后重启Nginx容器或者重载配置。\
